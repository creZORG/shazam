...
import { getListingDetailsForAdmin, updateListingStatus, generateManualTicketForAdmin, getProductsForSelect, updateListingMerch } from './actions';
...
function FreeMerchManager({ listing }: { listing: ListingWithId }) {
    const { toast } = useToast();
    const router = useRouter();
    const [products, setProducts] = useState<Product[]>([]);
    const [selectedProductId, setSelectedProductId] = useState<string>(listing.freeMerch?.productId || '');
    ...
    const handleUpdateMerch = async () => {
        startSaving(async () => {
            let merchData: { productId: string; productName: string } | null = null;
            if (selectedProductId) {
                const product = products.find(p => p.id === selectedProductId);
                if (product) {
                    merchData = { productId: product.id, productName: product.name };
                }
            }
            
            const result = await updateListingMerch(listing.id, listing.type as 'event' | 'tour', merchData);
            if(result.success) {
                toast({ title: 'Promotion Updated!', description: 'Free merchandise offer has been updated.' });
                router.refresh();
            } else {
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center gap-2"><Gift /> Free Merchandise Promotion</CardTitle>
                <CardDescription>Attach a free shop item to every ticket purchased for this event. Leave as "None" to disable.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <Select onValueChange={setSelectedProductId} defaultValue={selectedProductId}>
                    <SelectTrigger>
                        <SelectValue placeholder="None (No Free Merchandise)" />
                    </SelectTrigger>
                    <SelectContent>
                        <SelectItem value="none">None (No Free Merchandise)</SelectItem>
                        {products.map(p => (
                            <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>
                        ))}
                    </SelectContent>
                </Select>
                 <Button onClick={handleUpdateMerch} disabled={isSaving} className="w-full">
